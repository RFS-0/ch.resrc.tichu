...
/**
 * Represents the Json body of a REST request or response that is derived from some object representation. The object
 * representation
 ...
 * valid Json. It does not hold any object representation of the REST body. But it is able to parse the String into an
 * object
 ...
 * REST resources. No container such as Spring is required for that. This allows us to fully unit test the parsing and
 * marshalling
 * behaviour of our REST resources without launching a container. At the same time we can still communicate the
 * structure of the REST
 * bodies in the method signatures of the REST controller through the type parameter of this object</p>
 *
 * <p>Method signatures are designed in such a way that it is impossible to parse or marshal the body into anything
 * else
 ...
 * instantiate a {@code JsonBody} from the REST request and serialize it from the REST response. Such message converters
 * are simple.
 * They just use the {@code toString} method to get the response body for output. Or they instantiate a {@code
 * JsonBody.Input},
 * providing the string body of the request to it and a {@link Json} implementation. The actual parsing or marshalling
 * is done by this
...
public abstract class JsonBody<T> {
  ...
  public T parse(Class<T> type) {
    return json.parsingResult(Input.of(content(), "body"), type).getOrElseThrow(InvalidInputDetected::of);
  }

  public Either<ValidationError, T> parsingResult(Class<T> type) {
    return json.parsingResult(Input.of(content()), type);
  }

  public Map parseMap() {
    return json.parsingResult(Input.of(content(), "body"), Map.class).getOrElseThrow(InvalidInputDetected::of);
  }
