...
public class ValidationErrorModifier {
  ...
  /**
   * Returns an operator that prepends the given text to the error details of a validation error. The text serves as context for the
  ...
  /**
   * Returns an operator that prepends the name of the given class to the error details of a validation error. The class name serves as
   * context for the remaining error details. The class name is converted to lower case with camel case replaced by spaces.
   * <p>
   * Normally, this operator is used for domain classes whose names have direct meaning to clients. Don't use this operators for
   * classes whose names are not understood by the target audience of the error messages.
   * </p>
   *
  ...
  /**
   * Returns an operator that replaced the error details with the text 'must be specified'. This is a recurring error message
   * stereotype. It serves to ensure consistent use of this error message.
   *
   * @return the operator, as explained.
   */
  ...
  /**
   * Returns an operator that replaced the error details with the text 'cannot be undefined'. This is a recurring error message
   * stereotype. It serves to ensure consistent use of this error message.
   *
   * @return the operator, as explained.
   */
  ...
  /**
   * Returns an operator that replaces the value origin with the specified text if the origin was unknown, so far. Does nothing if the
  ...
  /**
   * Returns an operator that prepends the specified index to the error details if the error type is {@link ValidationError}. Has no
   * effect for other error types.
   * <p>
   * Use this operator to supply index information if the modified validation validates elements of a collection.
   * </p>
   *
  ...
  /**
   * Returns an operator that applies all the specified operators to the error. The operators are applied in the order in which they
  ...
  public static <E> UnaryOperator<E> chained(UnaryOperator<E>... operators) {
    ...
    return (E error) -> folded(error, List.of(operators), (err, op) -> op.apply(err));
  }
}
