<template>
  <table>
    ...
    <tbody>
    <tr v-for="grandTichuStatistic in grandTichuStatistics" :key="grandTichuStatistic.team">
      <td>{{ grandTichuStatistic.name }}</td>
      <td>{{ grandTichuStatistic.count }}</td>
      <td>{{ grandTichuStatistic.succeededCount }}</td>
      <td>{{ grandTichuStatistic.probabilitySuccessful }}</td>
      <td>{{ grandTichuStatistic.expectedPointsPerRound }}</td>
    </tr>
    </tbody>
  </table>
</template>
...
export default class GrandTichuStatistics extends Vue {
  game!: Some<Game>;

  get grandTichuStatistics(): GrandTichuStatistic[] {
    const grandTichuStatistics = [] as GrandTichuStatistic[];
    this.game.value.teams.forEach(team => {
      ...
      const allTichus = this.game.value.rounds
        .map(round => round.tichus.get(firstPlayerId))
        .concat(this.game.value.rounds.map(round => round.tichus.get(secondPlayerId)))
        .filter(
          tichu => !!tichu &&
            (tichu.valueOf() === Tichu.GRAND_TICHU_FAILED.valueOf() || tichu.valueOf() === Tichu.GRAND_TICHU_SUCCEEDED.valueOf()),
        ) as Tichu[];
      const succeededCount = allTichus.filter(tichu => tichu.valueOf() === Tichu.GRAND_TICHU_SUCCEEDED).length || 0;
      const failedCount = allTichus.filter(tichu => tichu.valueOf() === Tichu.GRAND_TICHU_FAILED).length || 0;
      const probabilitySuccessful = Math.round((succeededCount / allTichus.length || 0) * 100) / 100;
      const expectedPointsPerRound = Math.round(((succeededCount * 200) - (failedCount * -200) / totalNumberOfRounds) * 100) / 100;
      grandTichuStatistics.push(
        {
          name: teamName,
          count: allTichus.length,
          succeededCount,
    ...
    [
      ...
      const succeededCount = allTichusOfPlayer.filter(tichu => tichu.valueOf() === Tichu.GRAND_TICHU_SUCCEEDED).length || 0;
    ...
    return grandTichuStatistics;
  }
}

interface GrandTichuStatistic {
  name: string;
  count: number;
