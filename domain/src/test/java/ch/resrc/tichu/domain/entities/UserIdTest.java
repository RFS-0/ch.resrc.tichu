...
class UserIdTest {
    ...
    void aValidUuid_resultOf_success(String validUuid) {
        // given, when:
        var result = UserId.resultOf(validUuid);

        // then:
        result.failureEffect(System.out::println);
        assertThat(result.isSuccess()).isTrue();
        assertThat(result.value().getPrimitiveValue()).isEqualTo(validUuid);
    }
    ...
    void aBlankUuid_resultOf_failure(String blankUuid) {
        // given, when:
        var result = UserId.resultOf(blankUuid);
    ...
    void anInvalidUuid_resultOf_failure(String invalidUuid) {
        // given, when:
        var result = UserId.resultOf(invalidUuid);
